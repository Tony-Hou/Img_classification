...
def train():
    ...
    with tf.Graph().as_default() as graph:
        ...
        def train_step(sess, train_op, global_step, img, lab):
            ...
            total_loss, global_step_count, _ = sess.run([train_op, global_step, metrics_op],feed_dict={x: img, y_true: lab})
        ...
        with sv.managed_session(config=config) as sess:
            for step in xrange(num_steps_per_epoch * num_epochs):
                ...
                if step % num_batches_per_epoch == 0:
                    ...
                    logits_value, probabilities_value, predictions_value = sess.run([logits, probabilities, predictions], feed_dict={x: image, y_true: label})
                ...
                if step % 10 == 0:
                    loss, _ = train_step(sess, train_op, sv.global_step, image, label)
                    summaries = sess.run(my_summary_op, feed_dict={x: image,  y_true: label})
